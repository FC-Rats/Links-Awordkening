...
if (isset($_SESSION['score']) && !empty($_SESSION['score'] && isset($_SESSION['wordList']) && !empty($_SESSION['wordList']))) {
    $score = $db->query(
        "UPDATE LA_SCORE SET score = :score, words = :words WHERE idGame = :idGame",
        array(
            array(":score", $_SESSION['score']),
            array(":idGame", $game[0]['id']),
            array(":words", implode(',', $_SESSION['wordList']))
        )
    );
}
...
<body>
    ...
    <div id="main-content">
        <a href="new-game.php">
            <button
                class="btn rounded-5 bg-danger m-2 m-md-5 px-4 py-2 py-md-3 text-white fs-5 fs-md-4 beautiful-button">
                Quitter la partie
            </button>
        </a>
        <div class="mx-5 my-2 game-and-score rounded-4 p-3  text-white">
            <div class="game m-5 text-center">
                <div class="d-flex flex-wrap justify-content-evenly align-items-center">
                    ...
                    <div class="text-center mb-4">
                        ...
                        <h2 class="fs-1">
                            <?= isset($_SESSION['score']) ? $_SESSION['score'] : 0 ?>
                        </h2>
                    </div>
                    <div class="text-center mb-4">
            ...
            <form action="../Game/addWord.php" method="post" class="text-center mx-5 d-flex justify-content-center">
                ...
                <div class="container mt-5">
                    ...
                    <?php } elseif ($_SESSION['relevantWord'] && empty($_SESSION['errorCode'])) { ?>
                        <div class="alert alert-danger bg-warning rounded-5 d-inline-block">
                            <span class="text-white">
                                Le mot que vous avez entré a fait un moins bon score :'(
                            </span>
                        </div>
    ...
    <script>
        var inputWord = document.getElementById('mot');

        if (inputWord) {
            // Utilisez la méthode scrollIntoView() pour faire défiler la page jusqu'à l'élément input
            inputWord.scrollIntoView();
            // Utilisez la méthode focus() pour sélectionner l'élément input
            inputWord.focus();
        }
    </script>
