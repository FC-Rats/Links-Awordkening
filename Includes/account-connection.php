...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["username"]) && isset($_POST["password"])) {
        ...
        if (isValidUsername($username) && isValidPassword($password)) {
            ...
            $connectionVerif = $db->query("SELECT id, username, password, verified, admin FROM LA_USER WHERE username = :username", array(array(":username", $username)));
            if (count($connectionVerif) == 1) {
                if (password_verify($password, $connectionVerif[0]['password']) && $connectionVerif[0]['verified'] == 1) {
                    ...
                    $_SESSION["isAdmin"] = $connectionVerif[0]['admin'];
