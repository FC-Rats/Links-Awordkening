    ...
    return (
        ...
        <Stack spacing={2} direction="row" flexWrap="wrap" justifyContent="center" alignItems="center">
            ...
            {scores.length > 1 && scores[1] && (
                <CaseHomePage title={"#2"} pseudo={context?.user?.name === scores[1].username || scores[1].visibility === 'PUBLIC' ? scores[1].username : "PROFIL PRIVÉ"} value={scores[1].totalScore} color={'var(--coffee)'} />
            )}
            {scores.length > 2 && scores[2] && (
                <CaseHomePage title={"#3"} pseudo={context?.user?.name === scores[2].username || scores[2].visibility === 'PUBLIC' ? scores[2].username : "PROFIL PRIVÉ"} value={scores[2].totalScore} color={'var(--hunterGreen)'} />
            )}
        </Stack>
        </>
    );
